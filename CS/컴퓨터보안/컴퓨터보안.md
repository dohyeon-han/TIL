...
- 복소수, 실수, 유리수 집합은 field가 될 수 있지만, 정수 집합은 곱셈에 대한 역원이 모두 존재하는 것이 아니기 때문에 field가 될 수 없다.
...
- S-box는 16X16으로 0~15까지의 값을 행과 열로 하여 GF(2<sup>8</sup>)을 이용해 역원을 구하여 행렬, XOR 연산을 통해 미리 만들어 낼 수 있다.
- 역원은 선형적인 계산이 아니기 때문에 키값을 유추하기 어렵게 만든다.

### Shift Rows
- 맨 위의 행부터 각각 왼쪽으로 1칸, 2칸, 3칸, 4칸 씩 옮긴다.
...
- 키를 두 개 혹은 세 개를 사용하는데, 가운데 연산은 반대로 하여 DES를 한 번 적용하는 곳에서도 같은 키를 사용하면 Triple DES를 사용할 수 있는 하위 호환성을 보장하였다.
...
5. CTR(Counter) Mode
    ...
    - 병렬화, 선처리, 같은 입력에 대해 다른 결과 값이 모두 가능하다.
    - 에러 전파는 되지 않는다.
...
- entroy source를 이용해 난수를 생성한다.
- key stroke, 마우스의 움직임, ring oscillator 등 주로 아날로그 신호를 통해 생성할 수 있다.
